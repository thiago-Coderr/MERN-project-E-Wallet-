...
function SendEUR({ send }) {
  ...
  function onSubmit(data) {
    axios
      .post("http://localhost:8000/auth/sendEur", {
        email: data.email,
        amount: data.amount,
      })
      .then((res) => {
  ...
  return (
    <div className={styles.send_container}>
      ...
      <div onClick={handleSendEur} className={styles.send_container_iconClose}>
        <IconClose />
      </div>
      <div className={styles.send_container_content}>
        {success ? (
          ...
          <>
            <h2 className={styles.title}>Send EUR</h2>
